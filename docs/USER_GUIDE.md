...
- **Mehrere Track-Farben (seit v0.9.435)** — die Track-Linie kann ab bestimmten Distanzen die **Farbe wechseln**. In der Sektion **„Mehrere Track-Farben"** setzt du Farb-Stops **ab km** (Zahl), **an der aktuellen Marker-Position** (übernimmt die Scrubber-Position) oder **an allen GPX-Wegpunkten** (automatisch). Pro Stop stellst du **km + Farbe** ein, 🗑 entfernt ihn. Der Schalter **„Übergang"** legt fest, ob die Farbe **hart** (auf einen Schlag) oder als weicher **Verlauf** wechselt. Die erste Farbe gilt ab km 0 (= Track-Farbe). Wirkt WYSIWYG in Vorschau, Probelauf und Render. Standard aus. *(Aktuell nur Einzeltrack-Animator.)*
