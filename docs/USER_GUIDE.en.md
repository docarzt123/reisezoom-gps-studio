...
### Map sources & keys — where the maps come from, what they cost, what you may publish 🗺️

**In short:** without any key you get satellite imagery, maps and 3D terrain — and you may publish the videos. You only need keys for **MapTiler** (free) and **Mapbox** (free, but videos only with purchased rights).

| Source | Key? | Cost | Publish videos? | Attribution in the picture |
|---|---|---|---|---|
| **Satellite (free)** — official aerial imagery | no | free | yes | automatic (e.g. "Aerial imagery: PNOA © IGN España") |
| **OpenFreeMap** (Liberty, Bright, Positron) | no | free, no limit | yes | automatic (© OpenMapTiles, © OpenStreetMap) |
| **OpenStreetMap, OpenTopoMap, CyclOSM, Humanitarian** | no | free (community tile servers, please no bulk use) | yes | automatic |
| **3D terrain** (AWS Terrain Tiles / Mapzen) | no | free | yes | automatic ("Terrain: Mapzen/AWS Terrain Tiles") |
| **MapTiler** (Satellite, Hybrid, Outdoor, Topo, Streets + its own terrain) | **yes** | free plan (100,000 tile requests/month, **non-commercial only**); Flex from about $30/month | **yes, up to 100,000 subscribers** (Cloud Terms §5) | automatic (© MapTiler © OpenStreetMap contributors) |
| **Mapbox** (Satellite 3D, Satellite + streets, Outdoors, Streets, Light, Dark) | **yes** (token `pk.…`) | free tier 50,000 map loads/month, credit card at signup | **only with purchased video rights** (Product Terms §1.7) | automatic (© Mapbox © OpenStreetMap) |

> ⚠️ **The attribution at the bottom right of the picture is part of the video.** Please don't crop it away — every source requires it, Mapbox and MapTiler explicitly.

> ⚠️ **My understanding, not legal advice.** Everything in this chapter — table, recommendations, hints in the app — is my reading of the providers' terms as of September 2026. Terms change, and I may be wrong. Read them yourself before publishing and decide for yourself:
...
#### Which official aerial imagery is behind "Satellite (free)"?
...
- **Europe:** Austria (basemap.at, CC BY 4.0), Switzerland (swisstopo), Luxembourg (CC0), Netherlands (PDOK, CC BY 4.0), France (IGN Géoplateforme), Spain incl. the Canaries (PNOA © IGN, CC BY 4.0), Portugal (DGT, CC BY 4.0), Italy (Geoportale Nazionale, 2012), Czechia (ČÚZK), Poland (GUGiK), Estonia (Maa-amet, CC BY 4.0), since 17 Sept 2026 Belgium (Flanders: Digitaal Vlaanderen; Wallonia: SPW, CC BY 4.0) and Slovakia (ÚGKK SR/GKÚ, CC BY 4.0); since 18 Sept 2026 Slovenia (GURS, CC BY 4.0). With your own free key (Settings → Maps → **Country keys**): Denmark (Dataforsyningen), Finland (Maanmittauslaitos), New Zealand (LINZ) — without a key Sentinel-2 stays there and the style note tells you. Outside Europe: USA, Japan and New South Wales, Australia (Spatial Services, CC BY 4.0).
...
#### Getting a MapTiler key (free, 3 minutes)
1. Create an account at [cloud.maptiler.com](https://cloud.maptiler.com/) — e-mail + password or Google/GitHub, **no credit card**.
2. Click the confirmation e-mail.
3. In the left menu open **API Keys** (direct: [cloud.maptiler.com/account/keys](https://cloud.maptiler.com/account/keys/)). A key is already there — copy it (20 characters, letters and digits).
4. In the app: **Settings → Map providers → MapTiler key**, paste → Save. The app reloads and the MapTiler styles appear in the list.

> ⚠️ **Leave "Allowed HTTP origins" empty on the key.** If you enter a website there, MapTiler locks out the render (it runs without a browser address) — the preview would work, the video would stay black.
>
> 💡 **Free plan:** 100,000 tile requests per month, enough for many renders. It is limited to **non-commercial use** — a monetised channel needs the Flex plan (about $30/month). Videos for channels up to 100,000 subscribers are allowed under MapTiler Cloud Terms §5; above that, ask MapTiler first.

#### Getting a Mapbox token (free — but read the note first)
Mapbox permits **publishing videos** containing its map content **only with purchased video rights** (Product Terms §1.7 "Print or Video Use"). For YouTube, Instagram etc. pick a free style or MapTiler. The token still makes sense if you use the Mapbox styles privately or have bought rights.

1. Create an account at [account.mapbox.com](https://account.mapbox.com/auth/signup) — since mid-2026 Mapbox requires a **credit card** at signup; nothing is charged within the free tier.
2. Click the confirmation e-mail.
3. In the dashboard open [Access tokens](https://account.mapbox.com/access-tokens/).
4. Copy the "Default public token" — it starts with `pk.eyJ…`
5. In the app: **Settings → Map providers → Mapbox token**, paste → Save.

> 💡 Free tier: 50,000 map loads per month. Before a video render with a Mapbox style the app reminds you of the rights once per session — it blocks nothing.

**Change keys later**: macOS menu → **Reisezoom** → **Settings…** (or Cmd+,) — Windows/Linux: ⚙ button top right → **Map providers**.
...
- **Highlights as signs** (tick, on by default): peaks, passes, viewpoints, waterfalls, huts,
  ...
  once, spread along the tour, one per 5 km, eight to sixteen. Plus, from the track itself, the
...
4. **In the module:** Animator, Tour Map, data animator and web map have a **template bar at the top of the
   sidebar** (“🧩 Apply template…” · 💾 “Save as template…”; no selection field, a template is only the starting point) for the current project.
...
**preview images** of your photos plus the **images of your signs** — the original photos stay with you.
...
it, the **⋯ button** on the collection (or a right-click) opens rename, show, “All in
Animator”, duplicate and delete. **Duplicate**
...
> want to wait, **"↻ Try again"** in the notice checks right away. Afterwards the app says what it is doing: "Step 1 of 2 — looking through the folders" (compares only, opens nothing) and "Step 2 of 2 — reading the new files" (capture time, location, camera, preview — originals stay untouched), each with a count and the folder it is at. A result line follows; if there is nothing to do, it says so. The automatic check (every six hours) only asks folders in which something changed — minutes instead of half an hour on a network drive; once a week, and always via the "Read now" button, it looks at every file.
>
...
- **Waypoint signs (since v0.9.171, fully customizable since v0.9.179)** — place text signs along the route (e.g. "Summit reached!"). The **"🚩 Signs"** area in the sidebar:
  ...
  - **Behavior & timing:** "Grow with zoom" on/off, **"Show the whole time"** (continuously visible), **lead-in** X sec. (appears earlier) + **"Stays visible"** X sec. (how long it stays after the marker reaches it; 0 = stays until the end), **fade** (hard/fade-in). Otherwise it appears in the video exactly when the marker reaches the point; it stands upright facing the camera. **Since v0.9.204:** A sign right at the start of the track with a **lead-in** now already appears **in the intro** (lead-in = 1 sec → shows up in the last intro second, instead of popping up only at the track start).
...
- **🎥 Steady camera in one section only** (since v0.9.534) — click the transition symbol between two keyframes to open “Transition to next keyframe”; it now has a checkbox “🎥 Smooth camera in this section”. The terrain smoothing then applies only between these two keyframes (with a short blend at the edges) — handy when a tour runs partly over mountains, partly on the flat. The symbol shows a small 🎥. The sidebar checkbox still applies to the whole video.
...
- **Keyframes stick to the route, not to the clock** (since v0.9.511): a
  ...
  with the landscape, not with the second.
  ⚠️ Projects from older versions that used trimming will show their camera
  move at a different spot — set those keyframes once more.
...
**Formats for social media (since 25.09.2026):** below the resolutions there is a second row **Insta 1:1** (1080×1080), **Insta 4:5** (1080×1350) and **Story 9:16** (1080×1920). **"Open as Tour-Map"** in the Animator takes over map style and bearing and fits the same area into the Tour-Map format; the bearing slider shows the same value as the map.
...
- **📥 Save section A→B as a new tour in the archive …** (since v0.9.718) — set anchors A and B, click, enter a name: the points from A to B (with times and sensors) become a **separate new tour in the archive**. The track in the inspector stays as it is. Afterwards: *Show in archive* (the new tour is selected there), *Keep working here* or *Remove again* (trash). Typical case: a drive and a hike in one file — the hike becomes its own tour. To get it out as a file, use **File → Export as GPX**.
...
**Editing (since v0.9.709):** every entry has a **⋯ menu** (also right-click in the list or on a block in the timeline): *Rename*, *Note*, *Change type* (drive, ferry, hike, walk, bike, run, water sport, break), *Split activity* — here (in the timeline, at the mouse), at anchor A, at a time or in the middle; only the entry is split, never the track —, *Merge with previous/next*, *Delete* — the deleted part merges into its neighbour. **Drag boundaries:** grab the transition between two blocks in the timeline and move it. **＋ A→B** turns the part between anchors A and B into an entry of your own, **📍 Point** sets a point of your own (with a name) with the next click on the map. Everything is one **⌘Z** step; your hand-made changes stay even after ↻ (recognise again).
...
**⤢ Large window:** the whole logbook as a draggable, resizable table — columns time, type, name, place, duration, km, climb, pace, sortable by clicking the header; ticks for multi-select (shift-click for a range) with a bar *Change type · Merge · Delete*; double-click a name to rename; clicking a row selects the entry on the map; ⋯ and right-click as in the list. Since v0.9.714 it is a real window: drag it by the title bar, pull any edge or corner to resize, double-click the title bar (or ▢) to maximise and restore; ⤢ opens and closes it, position and size are remembered.
...
**Confirming guessed types (since v0.9.713):** where detection was unsure, an entry carries the **“guessed”** mark. Click it — or right-click/⋯ in the list, timeline or on the map — and the top item is **“✓ Confirm bike”** (or whatever type is shown); you can still pick another type. Confirmed entries count as your own edits and survive ↻ re-detection. In the **large window** the **“guessed only”** tick shows just those entries; select all → **“✓ Confirm guessed”**. All undoable.

**On the map (since v0.9.713):** the highest point, your own points and sights appear as **round markers on the map**, findings as coloured dots. **Click a marker** — or the point in the list or timeline — to open a box that says **what it is** (e.g. “Sight from OpenStreetMap · castle”, “Highest point of the tour · 3,746 m”), with time, place and the buttons *Rename*, *Note*, *Delete*; a sight that is not yet in the logbook gets *Add to logbook*. A **finding** says what is broken, what the repair would do, and asks right away: **🩹 Repair** (affects all spots of this kind, undoable), *Next spot*, *This is fine*. **Right-click the track** for the same menu as in the timeline (rename, type, split activity here, merge …) plus *Set your own point here* and *Anchor here*. **Zoom coupling:** zoom in the timeline and the map shows the same stretch, highlighted; pan or zoom the map and the timeline shows the visible stretch. **Unticking “POIs”** removes the sights everywhere — timeline row, list and map. A deleted automatic point (including a POI) does not come back when the logbook is recognised again.
