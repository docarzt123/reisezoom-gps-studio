...
**Papelera de GPS Studio (desde v0.9.718).** Lo que GPS Studio borra o sobrescribe — una versión de ruta, un estado de proyecto, un archivo sustituido — pasa antes a la carpeta `_papierkorb` de la carpeta de la app (macOS: `~/Library/Application Support/Reisezoom GPS Studio`, Windows: `%APPDATA%\Reisezoom GPS Studio`). Allí se queda 14 días; `HERKUNFT.txt` indica de dónde venía.
...
Los interruptores de **«Mostrar en el mapa»** (lugares, carreteras, POI, transporte, fronteras) también funcionan aquí: sobre las ortofotos se colocan las capas de rotulación de OpenFreeMap (datos de OpenStreetMap, gratis, sin clave), iguales en la vista previa y en el vídeo. No en las exportaciones de mapa web. Si las ortofotos se ven apagadas, usa el grupo **«Aspecto de la ortofoto»**: saturación, contraste, brillo y tono (25/8/0/0 de fábrica, «Estándar» restablece); solo afecta a las ortofotos, igual en la vista previa, el vídeo y el mapa web. Desde el 17.09.2026 hay además **Relieve** (luz y sombra de los datos del terreno sobre las ortofotos, 0 de fábrica; los estilos lo activan), **Quitar bruma** (elimina el velo azul de las imágenes de satélite, 0 de fábrica) y arriba el selector **Estilo**: «Natural», «Intenso» o «Cinematográfico» ajustan todos los controles a un conjunto afinado; si luego mueves un control, vuelve a mostrar «Personalizado». Relieve y bruma se aplican a la vista previa y al vídeo, no al mapa web.
...
> ⚠️ **Mi interpretación, no asesoramiento legal.** Todo lo de este capítulo — tabla, recomendaciones, avisos en la app — es mi lectura de las condiciones de los proveedores a fecha de septiembre de 2026. Las condiciones cambian y puedo equivocarme. Lee tú mismo antes de publicar y decide tú:
...
- **Europa:** Austria (basemap.at, CC BY 4.0), Suiza (swisstopo), Luxemburgo (CC0), Países Bajos (PDOK, CC BY 4.0), Francia (IGN Géoplateforme), **España con Canarias (PNOA © IGN, CC BY 4.0)**, Portugal (DGT, CC BY 4.0), Italia (Geoportale Nazionale, 2012), Chequia (ČÚZK), Polonia (GUGiK), Estonia (Maa-amet, CC BY 4.0), desde el 17.09.2026 Bélgica (Flandes: Digitaal Vlaanderen; Valonia: SPW, CC BY 4.0) y Eslovaquia (ÚGKK SR/GKÚ, CC BY 4.0); desde el 18.09.2026 Eslovenia (GURS, CC BY 4.0). Con tu propia clave gratuita (Ajustes → Mapas → **Claves por país**): Dinamarca (Dataforsyningen), Finlandia (Maanmittauslaitos), Nueva Zelanda (LINZ); sin clave sigue Sentinel-2 y el aviso del estilo lo indica. Fuera de Europa: EE. UU., Japón y Nueva Gales del Sur, Australia (Spatial Services, CC BY 4.0).
...
queda donde está, los duplicados se omiten) y lo lee al momento. **Desde v0.9.713** la ventana se cierra, el archivo salta a «Archivo de rutas → Todas las rutas» (se quitan colección, búsqueda y filtros) y muestra la ruta nueva arriba, seleccionada y con el detalle abierto a la derecha; con varios archivos quedan marcados todos los nuevos. Si el archivo ya la conoce, «Mostrar la ruta existente» te lleva a ella.
...
**¿Cuándo aparece la caja? (desde v0.9.724)** Lo más fácil es en la **línea de tiempo bajo el mapa**: haz clic en **«▸ Overlays»** y se abre una fila por caja. Cada **barra es el tiempo en que se ve la caja**. **Arrastra** la barra para moverla y sus **bordes** para cambiar inicio y final. Los dos **puntos blancos** de arriba son la **entrada** y la **salida**: arrástralos y la caja aparece o desaparece suavemente durante ese tiempo (lo muestra la cuña oscura). **Al final de la barra la caja ya no se ve**: la salida va antes. Mientras arrastras, la vista previa salta a ese punto y la línea de estado dice a qué está atado el borde: en la **intro**, a segundos desde el inicio del vídeo; **durante la animación, a un punto del track** («en el km 349,3»: se queda ahí aunque cambies el ritmo o la duración); en la **pausa final**, a segundos antes del final («2,0 s antes del final»: se mantiene 2 s antes del final aunque el vídeo se alargue). Ejemplo: mostrar los datos totales desde el segundo 25 y que desaparezcan 2 segundos antes del final: borde izquierdo a 25 s, borde derecho a «2 s antes del final», listo. **Doble clic o clic derecho** en una barra abre la ventana de la caja; ahí están los mismos valores como números («segundo desde el inicio del vídeo», «en un punto del track (km)», «segundos antes del final del vídeo», «durante») y la **duración de entrada y de salida** por separado. Una caja desactivada se ve gris y discontinua. Los **segundos ⏱ de la barra lateral** siguen a la barra; «hasta» es un segundo del vídeo (vacío = hasta el final). Si «hasta» queda antes de «desde», el campo se pone rojo. Los proyectos anteriores conservan sus tiempos; una salida configurada ahora va dentro del tramo, así que la caja desaparece una duración de transición antes que antes.
...
- **🎥 Cámara estable solo en un tramo** (desde v0.9.534) — haz clic en el símbolo de transición entre dos keyframes para abrir «Transición al siguiente keyframe»; ahora tiene la casilla «🎥 Cámara tranquila en este tramo». El suavizado del terreno actúa solo entre esos dos keyframes (con un breve fundido en los bordes) — útil cuando una ruta va en parte por montaña y en parte por llano. El símbolo muestra un pequeño 🎥. La casilla de la barra lateral sigue valiendo para todo el vídeo.
...
- **📥 Guardar tramo A→B como nueva ruta en el archivo …** (desde v0.9.718) — pon las anclas A y B, haz clic y escribe un nombre: los puntos de A a B (con horas y sensores) pasan a ser una **ruta nueva e independiente en el archivo**. El track del inspector no cambia. Después: *Ver en el archivo* (la nueva ruta queda seleccionada), *Seguir aquí* o *Quitar de nuevo* (papelera). Caso típico: trayecto en coche y caminata en un mismo archivo — la caminata se convierte en su propia ruta. Para sacarla como archivo: **Archivo → Exportar como GPX**.
...
**⤢ Ventana grande:** todo el cuaderno como tabla movible y redimensionable: columnas hora, tipo, nombre, lugar, duración, km, desnivel, ritmo, ordenables con clic en la cabecera; casillas para selección múltiple (Mayús+clic para un rango) con barra *Cambiar tipo · Unir · Eliminar*; doble clic en el nombre para renombrar; clic en una fila selecciona la entrada en el mapa; ⋯ y clic derecho como en la lista. Desde v0.9.714 es una ventana de verdad: se mueve desde la barra de título, se redimensiona desde cualquier borde o esquina, doble clic en la barra (o ▢) maximiza y restaura; ⤢ la abre y la cierra, posición y tamaño se recuerdan.
...
**Confirmar tipos supuestos (desde v0.9.713):** donde la detección no estaba segura, la entrada lleva la marca **«supuesto»**. Un clic en ella —o clic derecho/⋯ en la lista, la línea de tiempo o el mapa— ofrece arriba **«✓ Confirmar bici»** (o el tipo mostrado); puedes elegir otro tipo igualmente. Las entradas confirmadas cuentan como edición propia y sobreviven a ↻. En la **ventana grande**, la casilla **«solo supuestos»** muestra solo esas entradas; seleccionar todo → **«✓ Confirmar supuestos»**. Todo con ⌘Z.

**En el mapa (desde v0.9.713):** el punto más alto, los puntos propios y los lugares de interés aparecen como **marcas redondas en el mapa**, los hallazgos como puntos de color. **Un clic en una marca** —o en el punto de la lista o la línea de tiempo— abre un recuadro que dice **qué es** (p. ej. «Lugar de interés de OpenStreetMap · castillo», «Punto más alto de la ruta · 3.746 m»), con hora, lugar y los botones *Renombrar*, *Nota*, *Borrar*; un lugar que aún no está en el diario recibe *Añadir al diario*. Un **hallazgo** dice qué está roto, qué haría la reparación y pregunta enseguida: **🩹 Reparar** (afecta a todos los lugares de ese tipo, se deshace con ⌘Z), *Siguiente lugar*, *Está bien así*. **Clic derecho en el track** abre el mismo menú que en la línea de tiempo (renombrar, tipo, dividir actividad aquí, unir …) más *Poner un punto propio aquí* y *Ancla aquí*. **Zoom acoplado:** al hacer zoom en la línea de tiempo el mapa muestra el mismo tramo resaltado; al mover o hacer zoom en el mapa, la línea de tiempo muestra el tramo visible. **Quitar la casilla «POIs»** retira los lugares de interés en todas partes: fila, lista y mapa. Un punto automático borrado (también un POI) no vuelve al recalcular.
