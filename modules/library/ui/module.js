...
function mountLibrary(body, headerActions) {
  ...
  body.innerHTML = `
    <aside class="panel lib-nav" id="lib-panel">
      <div class="lib-nav-title">${T("library.section_projects", "Projekte")}</div>
      <nav class="lib-scopes">
        <button class="lib-nav-item" id="lib-proj-btn" type="button"
                title="${T("library.projects_hint", "Alle Arbeitsstände: woran war ich dran?")}">
          <span class="lib-nav-ico">🎬</span>
          <span class="lib-nav-lbl">${T("library.projects_all", "Alle Projekte")}</span>
          <span class="lib-nav-n" id="lib-proj-n"></span>
        </button>
      </nav>

      <div class="lib-nav-title" style="margin-top:14px;">${T("library.section_library", "Bibliothek")}</div>
  ...
  async function reloadStats() {
    ...
    fillActivityOptions();
    renderScopes();
    { const pb = document.getElementById("lib-proj-btn");
      if (pb) pb.onclick = () => projViewSetzen(!_projView); }
    // Q19/Q21: Die App startet im Archiv, Bereich „Projekte" — app.js setzt
  ...
  function renderScopes() {
    ...
    $("lib-scopes").querySelectorAll("[data-scope]").forEach(b => {
      b.onclick = () => {
        if (_projView) projViewSetzen(false);
  ...
  /* ══ PROJEKTE-Bereich (E1, IDEAS §39) ═══════════════════════════════════
   * „Das Archiv komponiert" wörtlich: Touren UND Projekte wohnen hier.
   * Karten je Projekt, sortiert nach zuletzt bearbeitet; „fertig" sinkt ans
   * Ende; nie angefasste auto-Projekte liegen eingeklappt darunter (Q10c).
   * Öffnen springt ins zuletzt benutzte Modul des Projekts, die Modul-Chips
   * sind direkt klickbar (Q22). */
  let _projView = false;
  let _projekte = [];
  ...
  async function renderProjekte() {
    ...
    const nEl = document.getElementById("lib-proj-n");
    if (nEl) nEl.textContent = _projekte.filter(p => !p.auto).length || "";
    ...
    box.innerHTML = `${filterChip}
      <div class="lib-proj-liste">${deine.map(karte).join("")
      ...
      ${autos.length ? `<details class="lib-proj-autos"><summary>${T("library.proj_autos", "Automatisch angelegt")} (${autos.length})<span class="gpxi-q" data-tip="${T("library.proj_autos_tip", "Beim Öffnen einer Tour entsteht automatisch ein Arbeitsstand. Sobald du darin etwas baust oder ihn umbenennst, wandert er nach oben zu deinen Projekten.")}">?</span></summary>
        <div class="lib-proj-liste">${autos.map(karte).join("")}</div></details>` : ""}`;
    initHelpTips(box);
   ...
   *  Ablauf und Modus stecken ja im Projekt. */
  ...
  function renderView() {
    ...
    const btn = document.getElementById("lib-proj-btn");
    if (btn) btn.classList.toggle("is-on", pv);
