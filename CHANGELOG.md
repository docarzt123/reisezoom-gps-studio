...
- Flickering line along terrain tile seams (preview and video, reported with the Schorfheide project): with MapLibre's default terrain mesh (128) small triangles at the T-junctions between neighbouring terrain tiles flip from frame to frame, even with a static camera (measured 370 changing pixels per frame). The terrain mesh is now 256 (26 pixels; 512 brings nothing more, render-texture quality has no influence). / Flimmernder Strich entlang der Gelände-Kachelnähte (Vorschau und Video, Schorfheide-Projekt): mit MapLibres Standard-Geländenetz (128) kippen an den T-Stößen zwischen Nachbarkacheln je Bild kleine Dreiecke, auch bei stehender Kamera (gemessen 370 Wechselpixel je Bild). Das Geländenetz ist jetzt 256 (26 Pixel; 512 bringt nichts mehr, die Textur-Qualität hat keinen Einfluss).
