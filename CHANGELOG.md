...
### Changed
- Render speed: 1080p 3.7× faster, 4K 2× faster. in render mode MapLibre no longer runs its 300 ms paint-property transitions and symbol cross-fades. Every frame step changes paint properties (progress, running dot), and the scene waited for the map to go idle, which only happened once the transition had run out. In addition, the globe projection's error-correction animation (MapLibre: 500 ms ramp, 700 ms "in transition") is instant in render mode (vendor patch `globeerr`); it stalled every third frame for about 750 ms. Measured (Schorfheide, MacBook): 1080p 562 → 153 ms per frame, 4K 675 → 336 ms (there the screenshot with supersampling, 224 ms, now dominates). / Render-Tempo: 1080p 3,7-mal, 4K doppelt so schnell. im Render-Modus laufen MapLibres 300-ms-Übergänge der Paint-Eigenschaften und die Symbol-Überblendung nicht mehr. Jeder Bildschritt setzt Paint-Eigenschaften (Fortschritt, Laufpunkt), und die Szene wartete auf „idle", das erst nach Ablauf des Übergangs kam. Zusätzlich läuft die Angleich-Animation der Globus-Fehlerkorrektur (MapLibre: 500 ms Rampe, 700 ms „im Übergang") im Render-Modus sofort (Vendor-Patch `globeerr`); sie hielt jedes dritte Bild etwa 750 ms an. Gemessen (Schorfheide, MacBook): 1080p 562 → 153 ms je Bild, 4K 675 → 336 ms (dort dominiert jetzt der Screenshot mit Supersampling, 224 ms).
