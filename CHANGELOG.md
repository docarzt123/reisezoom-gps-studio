...
- Attribution readable in the preview and the video: the source line at the bottom right is no longer shrunk with the preview scale (about 5 px tall in the preview and 12 px in a 1080p video, so you could not see where to place the watermark). It keeps a compact size in the preview and appears at the same relative size in the video. New setting "Source line" in the overlay section: corner (bottom right, bottom left, top right, top left) and width (narrow/tall, medium, wide/flat), so it no longer covers the elevation profile or the compass. The source line can never be switched off or collapsed: no checkbox, and MapLibre's collapse button is gone. / Quellenzeile lesbar in Vorschau und Video: die Nennung unten rechts wird nicht mehr mit dem Vorschau-Maßstab verkleinert (etwa 5 px in der Vorschau, 12 px im 1080p-Video, man sah nicht, wo das Wasserzeichen hin soll). Sie behält in der Vorschau eine kompakte Größe und erscheint im Video in derselben relativen Größe. Neue Einstellung „Quellenzeile" im Overlay-Abschnitt: Ecke (unten rechts, unten links, oben rechts, oben links) und Breite (schmal/hoch, mittel, breit/flach), damit sie Höhenprofil oder Nordpfeil nicht mehr verdeckt. Die Quellenzeile lässt sich nie abschalten oder zuklappen: kein Schalter, und MapLibres Zuklapp-Knopf ist weg.
