...
### Fixed
- Swarm/journey tracks flickered in 4K renders with 3D terrain (MapLibre): MapLibre drapes line layers onto the terrain mesh, so every ridge partially hides a track and the occlusion edge jitters while the camera moves. With terrain on, the swarm now draws its lines through a new custom layer (`ui/js/rz-line3d.js`) 150 m above the terrain, like Mapbox did with line-z-offset: screen-constant width, round joins, depth-tested against the mountains, globe and mercator projection. Without terrain the normal line layer stays. Verified headless: no more broken segments, residual frame-to-frame change is edge antialiasing only. / Schwarm-/Reise-Tracks flimmerten in 4K-Renders mit 3D-Gelände (MapLibre): MapLibre drapiert Linien auf das Geländenetz, jeder Grat verdeckt eine Spur teilweise, und die Verdeckungskante zittert bei Kamerabewegung. Mit Gelände zeichnet der Schwarm seine Linien jetzt über eine neue eigene Ebene (`ui/js/rz-line3d.js`) 150 m über dem Gelände, wie Mapbox mit line-z-offset: konstante Bildbreite, runde Verbindungen, Tiefentest gegen die Berge, Globus- und Merkator-Projektion. Ohne Gelände bleibt die normale Linien-Ebene. Kopflos geprüft: keine aufgebrochenen Segmente mehr, Restflimmern nur Kanten-Antialiasing.
