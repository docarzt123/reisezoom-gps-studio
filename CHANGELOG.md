...
- Preview quality (Settings → Maps, window only): «full» = like the video, «brisk» = the tile density from before 07.09 (a quarter of the tiles per frame), «fast» = additionally half the pixel scale on Retina and a coarser terrain mesh. Video and still always render at full quality; camera, track and overlays are identical. For previews that stutter (Masca 2 on a busy machine). — Vorschau-Qualität voll / flott / schnell, nur im Fenster.
