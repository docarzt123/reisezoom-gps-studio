...
### Added
- Sharpness slider (Karte → „Schärfe", 0–100 %, default 0): an unsharp mask on the map canvas, identical in preview and video (not in the web-map exports). Affects map and track, not numbers, profile or source line. Reason: the free Sentinel-2 imagery is soft over islands and coasts; sharpening at 40–60 % makes it readable without changing the data. — Schärfe-Regler (Karte → „Schärfe"): Unschärfemaske auf der Karten-Leinwand, Vorschau = Video.
